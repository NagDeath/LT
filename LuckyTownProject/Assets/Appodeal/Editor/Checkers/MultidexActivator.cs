...
namespace AppodealAds.Unity.Editor.Checkers
{
    public class MultidexActivator : CheckingStep
    {
        ...
        //public const string gradleTemplatesPath = appodealTemplatesPath + "/gradle_scripts";
            ...
            if (isNodexBuild() && EditorUserBuildSettings.activeBuildTarget == BuildTarget.Android)
            {
                if(!AppodealUnityUtils.isGradleEnabled()){
                    if(!AppodealUnityUtils.isGradleAvailable()){
                        FixProblemInstruction fix = new FixProblemInstruction("Gradle should be enabled if you use nodex version of Appodeal Plugin. But your version of Unity doesn't support gradle build system.Please update your Unity or use export to Eclipse or Android Studio.", false);
                        instructions.Add(fix);
                        return instructions;
                    }
                    else instructions.Add(new EnableGradle());
                }
                ...
                if(string.IsNullOrEmpty(customGradeScript) || !File.Exists(customGradeScript)) {
                    ...
                    if(File.Exists(getDefaultGradleTemplate())) instructions.Add(new CopyGradleScriptAndEnableMultidex());
                    else {
                        FixProblemInstruction instr = new FixProblemInstruction("Tere is no build.gradle template in your Unity. Please ensure that your copy of Unity isn't crashed and contact Appodeal Support team.", false);
                ...
                else
                {
                    ...
                    if (!settings.isEverythingFine()) instructions.Add(new EnableMultidexInGradle(customGradeScript));
                }
                string customManifestPath = getCustomManifestPath();
                if(string.IsNullOrEmpty(customManifestPath) || !File.Exists(customManifestPath)) instructions.Add(new CopyManifestTemplateAndAddNameAttribute());
                else{
                    XmlElement appNode = getApplicationNode(customManifestPath);
                    string ns = appNode.GetNamespaceOfPrefix("android");
                    if (!appNode.HasAttribute("name", ns)) instructions.Add(new AddNameAttrubute(appNode, ns));
                    else{
                        if (appNode.GetAttribute("name", ns) != manifestMutlidexApp){
                            FixProblemInstruction fix = new FixProblemInstruction("We found that you use custom Application class in this project. Please ensure that your application class meets the multidex requirements (see the official android multiex documentation).", false);
                            instructions.Add(fix);
                        }
                    }
                }
    ...
    public class ImportantGradleSettings
    {
        ...
        public ImportantGradleSettings(string gradlePath){
            ...
            muntidexEnabled = gradleScriptFullText.Contains(MultidexActivator.GRADLE_MULTIDEX_ENABLE);
        ...
        {
            return multidexDependencyPresented || muntidexEnabled;
        }
        public bool isEverythingFine()
        {
            return multidexDependencyPresented && muntidexEnabled && !deprecatedProguardPresented && googleRepositoryPresented;
        }
        private string getModule(string moduleName, string fulltext)
        {
            int startIndex = fulltext.IndexOf(moduleName);
    ...
    class CopyGradleScriptAndEnableMultidex : FixProblemInstruction{
        ...
        public override void fixProblem()
        {
            ...
            if (File.Exists(defaultGradleTemplateFullName)) File.Copy(defaultGradleTemplateFullName, destGradleScriptFullName);
            /*else
            {
                string gradleScriptAbsolutePath = Path.Combine(Application.dataPath, MultidexActivator.gradleTemplatesPath);
                string[] scriptVersions = Directory.GetDirectories(gradleScriptAbsolutePath);
                string finalVersion = "0.0.0";
                foreach(string versionPath in scriptVersions){
                    string version = Path.GetFileName(versionPath);
                    int unityVsScript = AppodealUnityUtils.compareVersions(Application.unityVersion, version);
                    if(unityVsScript >= 0){
                        int finalVsScript = AppodealUnityUtils.compareVersions(finalVersion, version);
                        if (finalVsScript < 0) finalVersion = version;
                    }
                }
                gradleScriptAbsolutePath = AppodealUnityUtils.combinePaths(gradleScriptAbsolutePath, finalVersion, MultidexActivator.gradleTemplateName);
                File.Copy(gradleScriptAbsolutePath, destGradleScriptFullName);
            }*/
            ...
            if (!settings.isEverythingFine()) new EnableMultidexInGradle(destGradleScriptFullName).fixProblem();
        }
    }

    class EnableMultidexInGradle : FixProblemInstruction{
        private string path;
        public EnableMultidexInGradle(string gradleScriptPath):base("Multidex isn't enabled. mainTemplate.gradle should be edited according to the official documentation:\nhttps://developer.android.com/studio/build/multidex", true){
        ...
        public override void fixProblem()
        {
            ...
            if (comparsionUnityVersionWith20182 < 0) multidexDependency = MultidexActivator.GRADLE_COMPILE + MultidexActivator.GRADLE_MULTIDEX_DEPENDENCY;
            else multidexDependency = MultidexActivator.GRADLE_IMPLEMENTATION + MultidexActivator.GRADLE_MULTIDEX_DEPENDENCY;
            ...
            while ((line = gradleScript.ReadLine()) != null)
            {
                ...
                if (!settings.muntidexEnabled && line.Contains(MultidexActivator.GRADLE_APP_ID))
                ...
                if (settings.deprecatedProguardPresented && line.Contains(MultidexActivator.GRADLE_USE_PROGUARD))
                    continue; //useProguard is deprecated
    ...
    class AddNameAttrubute : FixProblemInstruction
    {
        ...
        private string ns;
        public AddNameAttrubute(XmlElement appNode, string ns) : base("Problem in AndroidManifest.xml found.\nApplication tag should contain android:name attribute with value: " + MultidexActivator.manifestMutlidexApp, true) {
